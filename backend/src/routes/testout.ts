...
import { modules, Module, LocalizedText } from '../data/lessons';
...
const TRUE_FALSE_OPTS: LocalizedText[] = [
  { en: 'True', bg: 'Вярно' },
  { en: 'False', bg: 'Невярно' },
];

/** Deterministically build the test-out quiz from a module's own multiple-choice-style exercises. */
function buildTestOut(mod: Module): QuizQ[] {
  const out: QuizQ[] = [];
  const push = (q: QuizQ) => { if (out.length < MAX_QUESTIONS) out.push(q); };
  ...
  for (const lesson of mod.lessons) {
    for (const ex of lesson.exercises) {
      ...
      if (ex.type === 'choice' && ex.question && Array.isArray(ex.options) && typeof ex.correctIndex === 'number') {
        ...
        ex.speedRound.questions.slice(0, 3).forEach((sq, i) =>
          push({ exerciseId: `${ex.id}#${i}`, lessonId: lesson.id, prompt: sq.q, options: sq.options, correctIndex: sq.correctIndex }));
      } else if (ex.type === 'boss_battle' && ex.bossBattle?.questions) {
        ex.bossBattle.questions.slice(0, 3).forEach((sq, i) =>
          push({ exerciseId: `${ex.id}#${i}`, lessonId: lesson.id, prompt: sq.q, options: sq.options, correctIndex: sq.correctIndex }));
      }
    }
  }
...
function isModuleUnlocked(index: number, isPro: boolean, completed: Set<string>): boolean {
  const mod = modules[index];
  if (mod.proOnly && !isPro) return false;
  if (isPro || index === 0) return true;
  const prev = modules[index - 1];
  if (!prev || prev.proOnly) return true;
...
async function loadState(userId: number) {
  ...
  const completed = new Set<string>(progress.rows.map((r: { lesson_id: string }) => r.lesson_id));
  const isPro: boolean = userRow.rows[0]?.is_pro ?? false;
  return { completed, isPro };
}

/* ─── GET /api/testout/:moduleId — quiz WITHOUT answers ───────── */
testoutRouter.get('/:moduleId', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  const index = modules.findIndex((m) => m.id === req.params.moduleId);
  if (index < 0) { res.status(404).json({ error: 'module_not_found' }); return; }
  const mod = modules[index];

  try {
    const { completed, isPro } = await loadState(req.userId!);

    if (mod.proOnly && !isPro) { res.json({ eligible: false, reason: 'pro_required' }); return; }
    if (!isModuleUnlocked(index, isPro, completed)) { res.json({ eligible: false, reason: 'locked' }); return; }
    if (mod.lessons.every((l) => completed.has(l.id))) { res.json({ eligible: false, reason: 'completed' }); return; }

    const quiz = buildTestOut(mod);
    if (quiz.length < MIN_QUESTIONS) { res.json({ eligible: false, reason: 'no_quiz' }); return; }
...
testoutRouter.post('/:moduleId', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  const index = modules.findIndex((m) => m.id === req.params.moduleId);
  if (index < 0) { res.status(404).json({ error: 'module_not_found' }); return; }
  const mod = modules[index];
  ...
  try {
    const { completed, isPro } = await loadState(req.userId!);
    if (mod.proOnly && !isPro) { res.status(403).json({ error: 'pro_required' }); return; }
    if (!isModuleUnlocked(index, isPro, completed)) { res.status(403).json({ error: 'locked' }); return; }
    if (mod.lessons.every((l) => completed.has(l.id))) { res.status(400).json({ error: 'already_completed' }); return; }

    const quiz = buildTestOut(mod);
    if (quiz.length < MIN_QUESTIONS) { res.status(400).json({ error: 'no_quiz' }); return; }
    ...
    try {
      ...
      for (const lesson of mod.lessons) {
        await client.query(
          `INSERT INTO progress (user_id, lesson_id, module_id, xp_earned)
           VALUES ($1, $2, $3, 0)
           ON CONFLICT (user_id, lesson_id) DO NOTHING`,
          [req.userId, lesson.id, mod.id]
        );
      }

      const u = (await client.query(
        'SELECT xp, streak, last_active, streak_freezes, name FROM users WHERE id = $1 FOR UPDATE',
        [req.userId]
      )).rows[0] as { xp: number; streak: number; last_active: string | null; streak_freezes: number; name: string };

      const today = todayStr();
      const { newStreak: ns, newFreezes } = computeStreakUpdate(
        { streak: u.streak, last_active: u.last_active, streak_freezes: u.streak_freezes ?? 0 },
        today
      );
      oldXp = u.xp;
      name = u.name;
    ...
    const today = todayStr();
    detectCrossesAndNotify(req.userId!, name, oldXp, newXp).catch((e) => console.error('cross-XP notify failed:', e));
    updateFriendStreaksForUser(req.userId!, today).catch((e) => console.error('friend-streak update failed:', e));
    contributeToFriendQuests(req.userId!, TESTOUT_XP, today).catch((e) => console.error('friend-quest contribute failed:', e));
