...
export const CUP_TOTAL_SUPPLY = 3;

/** Length of the Pro trial granted by the wheel. */
export const PRO_TRIAL_DAYS = 14;

/**
 * Display slots shown on the wheel UI, ordered clockwise from the top.
 * `slotIndex` here determines where the pointer visually lands.
 * The backend reward draw maps to one of these slot ids.
 */
export type SlotId =
  | 'xp_25' | 'xp_50' | 'xp_100' | 'xp_200' | 'xp_500' | 'xp_1000'
  | 'cosmetic_common' | 'cosmetic_rare'
  | 'pro_trial' | 'cup';

interface Slot {
  id: SlotId;
  /** Weight for the random draw (total of all weights = 1000). */
  weight: number;
  type: 'xp' | 'cosmetic' | 'pro_trial' | 'cup';
  /** XP amount when type='xp'; rarity filter when type='cosmetic'; ignored otherwise. */
  value?: number | string;
}
...
const SLOT_LABELS: Record<SlotId, { en: string; bg: string; emoji: string }> = {
...
async function pickRandomCosmetic(userId: number | undefined, rarity: 'common' | 'rare'): Promise<string | null> {
  const pool = getPool();
  const owned = await pool.query<{ item_id: string }>(
    'SELECT item_id FROM user_inventory WHERE user_id = $1', [userId]
  );
  const ownedSet = new Set(owned.rows.map(r => r.item_id));
  const candidates = CATALOG.filter(c => c.rarity === rarity && !ownedSet.has(c.id));
  if (candidates.length === 0) {
    // Fallback to any rarity they don't own.
    const allUnowned = CATALOG.filter(c => !ownedSet.has(c.id));
    if (allUnowned.length === 0) return null;
    return allUnowned[crypto.randomInt(0, allUnowned.length)].id;
  }
  return candidates[crypto.randomInt(0, candidates.length)].id;
}
 ...
 * ───────────────────────────────────────────────────────────── */
wheelRouter.get('/info', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const pool = getPool();
    const r = await pool.query<{ wheel_spun: boolean }>(
      'SELECT wheel_spun FROM users WHERE id = $1', [req.userId]
    );
    ...
    res.json({ canSpin, slots, cupSupplyTotal: CUP_TOTAL_SUPPLY });
  } catch (err) {
 ...
 * ───────────────────────────────────────────────────────────── */
wheelRouter.post('/spin', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  const pool = getPool();
  ...
  try {
    ...
    const u = await client.query<{ wheel_spun: boolean; xp: number }>(
      'SELECT wheel_spun, xp FROM users WHERE id = $1 FOR UPDATE', [req.userId]
    );
    if (!u.rows[0]) {
    ...
    if (slot.id === 'cup') {
      const cupCount = await client.query<{ count: string }>(
        "SELECT COUNT(*)::text AS count FROM wheel_prizes WHERE reward_type = 'cup'"
      );
      const taken = parseInt(cupCount.rows[0].count, 10);
      if (taken >= CUP_TOTAL_SUPPLY) {
    ...
    const reward: {
      ...
      isCup?: boolean;
    } = { slotId: slot.id, type: slot.type };

    if (slot.type === 'xp') {
      const amount = slot.value as number;
      await client.query('UPDATE users SET xp = xp + $1 WHERE id = $2', [amount, req.userId]);
      reward.xpDelta = amount;
      await client.query(
        `INSERT INTO wheel_prizes (user_id, reward_type, reward_value) VALUES ($1, 'xp', $2)`,
        [req.userId, String(amount)]
      );
    } else if (slot.type === 'cosmetic') {
      const rarity = slot.value as 'common' | 'rare';
      ...
      if (itemId) {
        ...
        await client.query(
          `INSERT INTO wheel_prizes (user_id, reward_type, reward_value) VALUES ($1, 'cosmetic', $2)`,
          [req.userId, itemId]
        );
      } else {
        ...
        reward.xpDelta = 500;
        await client.query(
          `INSERT INTO wheel_prizes (user_id, reward_type, reward_value) VALUES ($1, 'xp', '500')`,
          [req.userId]
        );
      }
      ...
      await client.query(
        `UPDATE users SET is_pro = TRUE,
                          pro_trial_ends_at = NOW() + INTERVAL '${PRO_TRIAL_DAYS} days'
         WHERE id = $1`, [req.userId]
      );
      const endRow = await client.query<{ pro_trial_ends_at: string }>(
        'SELECT pro_trial_ends_at FROM users WHERE id = $1', [req.userId]
      );
      reward.proTrialEndsAt = endRow.rows[0].pro_trial_ends_at;
      await client.query(
        `INSERT INTO wheel_prizes (user_id, reward_type, reward_value)
         VALUES ($1, 'pro_trial', $2)`,
        [req.userId, `${PRO_TRIAL_DAYS}d`]
      );
