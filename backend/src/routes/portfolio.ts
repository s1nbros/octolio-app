...
interface Asset {
  ...
  name: string;
  emoji: string;
  category: 'stock' | 'etf' | 'crypto' | 'bond' | 'commodity';
  base: number;      // reference price at BASE_DAY
  drift: number;     // annualised trend (e.g. 0.09 = +9%/yr)
  vol: number;       // amplitude of the smooth wave (fraction of price)
  period: number;    // wave period in days
...
export function priceOn(asset: Asset, day: number): number {
...
async function ensureAccount(userId: number): Promise<number> {
  const pool = getPool();
  await pool.query(
    `INSERT INTO portfolio_accounts (user_id, cash) VALUES ($1, $2) ON CONFLICT (user_id) DO NOTHING`,
...
portfolioRouter.get('/', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const pool = getPool();
    const cash = await ensureAccount(req.userId!);
    const holdingsRows = (await pool.query(
      'SELECT asset_id, shares, avg_cost FROM portfolio_holdings WHERE user_id = $1 AND shares > 0',
      [req.userId]
    )).rows as Array<{ asset_id: string; shares: string; avg_cost: string }>;
    ...
    res.json({
      ...
      totalReturnPct: Math.round((totalValue / STARTING_CASH - 1) * 10000) / 100,
      holdings,
      market: marketToday(),
    });
  } catch (err) {
...
portfolioRouter.post('/trade', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  const { assetId, side, shares } = req.body as { assetId?: string; side?: 'buy' | 'sell'; shares?: number };
  ...
  try {
    ...
    const holdRow = (await client.query(
      'SELECT shares, avg_cost FROM portfolio_holdings WHERE user_id = $1 AND asset_id = $2 FOR UPDATE',
      [req.userId, asset.id]
    )).rows[0] as { shares: string; avg_cost: string } | undefined;
    const heldShares = holdRow ? Number(holdRow.shares) : 0;
    const heldAvg = holdRow ? Number(holdRow.avg_cost) : 0;

    if (side === 'buy') {
      const cost = qty * price;
      if (cost > cash + 1e-6) { await client.query('ROLLBACK'); res.status(400).json({ error: 'insufficient_funds' }); return; }
      cash -= cost;
      const newShares = heldShares + qty;
      const newAvg = newShares > 0 ? (heldShares * heldAvg + qty * price) / newShares : price;
      await client.query(
        `INSERT INTO portfolio_holdings (user_id, asset_id, shares, avg_cost) VALUES ($1,$2,$3,$4)
         ON CONFLICT (user_id, asset_id) DO UPDATE SET shares = $3, avg_cost = $4`,
        [req.userId, asset.id, newShares, newAvg]
      );
    } else {
      if (qty > heldShares + 1e-6) { await client.query('ROLLBACK'); res.status(400).json({ error: 'insufficient_shares' }); return; }
      cash += qty * price;
      const newShares = heldShares - qty;
      await client.query(
        `UPDATE portfolio_holdings SET shares = $3 WHERE user_id = $1 AND asset_id = $2`,
        [req.userId, asset.id, newShares]
      );
    }

    await client.query('UPDATE portfolio_accounts SET cash = $2 WHERE user_id = $1', [req.userId, cash]);
    await client.query(
      'INSERT INTO portfolio_trades (user_id, asset_id, side, shares, price) VALUES ($1,$2,$3,$4,$5)',
      [req.userId, asset.id, side, qty, price]
    );
