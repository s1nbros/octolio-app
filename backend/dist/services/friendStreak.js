...
exports.updateFriendStreaksForUser = updateFriendStreaksForUser;
 ...
 */
async function updateFriendStreaksForUser(userId, today) {
    ...
    const friends = (await pool.query(`SELECT u.id, u.name
       FROM friendships f
      ...
      WHERE f.status = 'accepted'
        ...
        AND u.last_active = $2`, [userId, today])).rows;
    if (friends.length === 0)
        return;
    ...
    for (const friend of friends) {
        ...
        const bumped = (await pool.query(`INSERT INTO friend_streaks (user_low, user_high, streak_count, best_streak, last_incr_date)
       VALUES ($1, $2, 1, 1, $3)
       ...
       RETURNING streak_count`, [low, high, today, yesterday])).rows[0];
        if (!bumped)
 ...
 */
function effectiveStreak(streakCount, lastIncrDate, today) {
    if (!lastIncrDate)
        return 0;
    if (lastIncrDate === today || lastIncrDate === prevDay(today))
        return streakCount;
    return 0;
}
