...
exports.clearSeenFingerprints = clearSeenFingerprints;
...
async function fetchTheorySeed(topic) {
...
// ─────────────────── 2. AUTHOR VIA CLAUDE ───────────────────
function buildPrompt(topic, seed, avoid) {
...
async function authorLesson(topic, seed, avoid) {
...
function fingerprint(ex) {
    const salient = (ex.question && ex.question.en) ||
    ...
    return crypto_1.default.createHash('sha1').update(`${ex.type}:${salient}`).digest('hex');
}
function dedupe(lesson) {
    const kept = [];
...
function validate(lesson) {
    if (!lesson.exercises?.length)
        throw new Error('No exercises produced');
    if (lesson.exercises[0].type !== 'theory')
        throw new Error('First exercise must be theory');
    const nonTheory = lesson.exercises.filter(e => e.type !== 'theory');
    if (nonTheory.length < 7)
        throw new Error(`Need ≥7 interactive exercises, got ${nonTheory.length}`);
    const kinds = new Set(nonTheory.map(e => e.type));
    if (kinds.size < 4)
        throw new Error('Need at least 4 distinct non-theory exercise types');
...
function clearSeenFingerprints() { seenFingerprints.clear(); }
async function generateModule(spec) {
    const lessons = [];
    for (let i = 0; i < spec.lessonTopics.length; i++) {
        ...
        lesson.order = i + 1;
        if (spec.lessonIcons?.[i])
            lesson.icon = spec.lessonIcons[i];
        lessons.push(lesson);
    }
    return {
        id: spec.id,
