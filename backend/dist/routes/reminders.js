...
function decide(lastActive, lastReminder, today) {
    if (!lastActive)
        return null;
    ...
    if (lastReminder === today)
        return null; // already emailed today
    if (daysSince >= 1 && daysSince <= STREAK_WINDOW_MAX)
        return 'streak';
    if (daysSince >= WINBACK_AFTER) {
        const sinceReminder = lastReminder ? dayDiff(lastReminder, today) : Infinity;
        if (sinceReminder >= WINBACK_COOLDOWN)
            return 'winback';
    }
...
exports.remindersRouter.post('/send', async (req, res) => {
    ...
    try {
        ...
        if (testEmail) {
            const r = await pool.query('SELECT email, name, streak FROM users WHERE email = $1', [String(testEmail).toLowerCase()]);
            const u = r.rows[0];
            if (!u) {
        ...
        const result = await pool.query(`SELECT id, email, name, streak, last_active, last_reminder_sent
         FROM users
        ...
        for (const u of result.rows) {
            ...
            if (type)
                todo.push({ id: u.id, email: u.email, name: u.name, streak: u.streak ?? 0, type });
        }
        const breakdown = {
        ...
        for (const t of todo) {
            try {
                if (t.type === 'winback')
                    await (0, email_1.sendWinbackEmail)(t.email, t.name, 'en');
                else
                    await (0, email_1.sendStreakReminderEmail)(t.email, t.name, t.streak, 'en');
