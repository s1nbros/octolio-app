...
exports.stripeRouter.post('/portal', auth_1.authenticate, async (req, res) => {
...
async function stripeWebhookHandler(req, res) {
    ...
    try {
        if (event.type === 'checkout.session.completed') {
            ...
            if (userId) {
                await pool.query('UPDATE users SET is_pro = TRUE, onboarding_done = TRUE, stripe_subscription_id = $1 WHERE id = $2', [session.subscription, userId]);
                console.log(`User ${userId} upgraded to Pro`);
            }
        }
        if (event.type === 'customer.subscription.deleted' || event.type === 'customer.subscription.paused') {
            const sub = event.data.object;
            await pool.query('UPDATE users SET is_pro = FALSE WHERE stripe_subscription_id = $1', [sub.id]);
            console.log(`Subscription ${sub.id} cancelled — user downgraded`);
        }
        if (event.type === 'customer.subscription.resumed') {
            const sub = event.data.object;
            await pool.query('UPDATE users SET is_pro = TRUE WHERE stripe_subscription_id = $1', [sub.id]);
        }
    }
    catch (err) {
