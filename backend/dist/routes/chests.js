 ...
 */
function chestUnlockAfterLessonIdx(numLessons, pos) {
    if (pos === 'end')
        return numLessons - 1;
...
function chestPositionsFor(numLessons) {
    if (numLessons <= 1)
        return ['end'];
    return ['mid', 'end'];
}
async function buildChestStates(userId) {
    const pool = (0, db_1.getPool)();
    // Get all completed lesson IDs for the user, indexed by module
    const progressRows = (await pool.query(`SELECT lesson_id, module_id FROM progress WHERE user_id = $1`, [userId])).rows;
    const completedByModule = new Map();
    for (const r of progressRows) {
        if (!completedByModule.has(r.module_id))
            completedByModule.set(r.module_id, new Set());
        completedByModule.get(r.module_id).add(r.lesson_id);
    }
    // Get all already-opened chest positions
    const openedRows = (await pool.query(`SELECT module_id, position FROM module_chests WHERE user_id = $1`, [userId])).rows;
    const openedSet = new Set(openedRows.map((r) => `${r.module_id}:${r.position}`));
    const out = [];
    for (const mod of lessons_1.modules) {
        const lessons = mod.lessons;
        const completedLessonIds = completedByModule.get(mod.id) ?? new Set();
...
exports.chestsRouter.post('/open', auth_1.authenticate, async (req, res) => {
    ...
    try {
       ...
       VALUES ($1, $2, $3, $4, $5)`, [req.userId, reward.type, rewardValue, coinsDelta, xpDelta]);
        ...
        await client.query(`INSERT INTO module_chests (user_id, module_id, position) VALUES ($1, $2, $3)
       ON CONFLICT (user_id, module_id, position) DO NOTHING`, [req.userId, moduleId, position]);
